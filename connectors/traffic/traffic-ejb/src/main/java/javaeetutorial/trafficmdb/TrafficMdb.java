...
@JMSDestinationDefinition(name = "java:app/traffic-ejb/traffictopic", interfaceName = "javax.jms.Topic", destinationName = "CityTopic", resourceAdapter = "traffic-rar")
/* Activate the traffic-rar resource adapter from this MDB */
@MessageDriven(activationConfig = { @ActivationConfigProperty(propertyName = "port", propertyValue = "4008") })
...
public class TrafficMdb implements TrafficListener {
	...
	public void processReport(String jsonReport) {
		...
		if (parser.hasNext() && parser.next() == JsonParser.Event.START_OBJECT && parser.hasNext()
				&& parser.next() == JsonParser.Event.KEY_NAME && parser.getString().equals("report") && parser.hasNext()
				&& parser.next() == JsonParser.Event.START_ARRAY && parser.hasNext()) {

			/*
			 * Parse array entries: {"city":"...", "access":"...",
			 * "status":"..."}
			 */
